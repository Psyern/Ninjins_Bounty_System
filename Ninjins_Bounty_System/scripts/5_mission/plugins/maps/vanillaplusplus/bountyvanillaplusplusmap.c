...
modded class VPPMapMenu
{
	...
	{
		super.Init();
		obfv_m_BountyMapWidget = MapWidget.Cast(layoutRoot.FindAnyWidget("Map_Widget"));
		if (!obfv_m_BountyMapWidget)
		{
			obfm_GetNinjins_Bounty_SystemLogger().obfm_LogWarning("[BountyVanillaPlusPlusMap] ERROR: MapWidget not found.");
			return layoutRoot;
		}
		...
		if (!canvasLayout)
		{
			obfm_GetNinjins_Bounty_SystemLogger().obfm_LogWarning("[BountyVanillaPlusPlusMap] ERROR: Failed to load canvas layout.");
			return layoutRoot;
		}
		obfv_m_BountyDrawCanvas = CanvasWidget.Cast(canvasLayout.FindAnyWidget("bountydrawCanvas"));
		if (!obfv_m_BountyDrawCanvas)
		{
			obfm_GetNinjins_Bounty_SystemLogger().obfm_LogWarning("[BountyVanillaPlusPlusMap] ERROR: drawCanvas not found in layout.");
			return layoutRoot;
		}
		...
		{
			obfm_GetNinjins_Bounty_SystemLogger().obfm_LogWarning("[BountyVanillaPlusPlusMap] ERROR: Failed to initialize BountyMapDrawer.");
		}
		return layoutRoot;
	}
